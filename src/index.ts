...
import { parseWithFriendlyErrors } from "@lunariajs/core/config";
...
async function getTrackedFilesTable(
  ...
  trackedFiles: Files,
  config: LunariaScopedConfig,
  status: LocalizationStatus[]
) {
  const { defaultLocale, locales, files, repository } = config;
  const rows: string[][] = [];
  ...
  for (const file of trackedFiles) {
    ...
    const key = `${isSourceLocale ? "source" : "localization"}-${statusType(
      file.status
    )}` as const;
    const note = `${notes[key]} ${warningIcons}`;

    rows.push([`[${rootlessFilename}](${file.blob_url})`, note, lang]);
  }

  const filesTable = markdownTable([["File", "Note", "Locale"], ...rows]);
  const warningsTable = markdownTable([
    ["Icon", "Description"],
    ...Object.values(warnings).map(({ icon, description }) => [
      icon,
      description,
    ]),
  ]);
...
async function main() {
  ...
  if (payload.action !== "opened" && payload.action !== "synchronize") {
    core.notice(
      "Skipped, Lunaria action only runs during pull request opening/synchronization."
    );
    return;
  }
  ...
  const trackedFiles = pullRequestFiles.filter((file) =>
    findFileConfig(
      unrootFilename(config.repository.rootDir, file.filename),
      config.files
    )
  );
  ...
  await commentSummary(
    octokit,
    pullRequestContext,
    body(overview, trackedFilesTable)
  );
}
